...
const AdminDashboard = () => {
  ...
  const fetchAllUsers = async () => {
    ...
    try {
      ...
      if (err.response?.status === 404) {
        setError("API endpoint not found (404). Please check backend route.");
      } else if (err.response?.status === 401) {
        setError("Session expired. Redirecting to login...");
        localStorage.removeItem('adminToken');
        setTimeout(() => window.location.href = '/admin-login', 1500);
      } else {
        setError(err.response?.data?.message || "Failed to load trainees. Please try again.");
      }
    } finally {
      setLoading(false);
    }
  ...
  const downloadUserDoc = async (traineeId) => {
    if (!traineeId) {
      setDownloadErrorMsg("Invalid trainee ID");
    ...
    try {
      ...
      if (err.response?.status === 404) {
        setDownloadErrorMsg("Download endpoint not found (404). Contact admin.");
      } else {
  ...
  const downloadAllPDF = async () => {
    try {
      ...
      if (err.response?.status === 404) {
        alert("Bulk download endpoint not found (404). Please check backend.");
      } else {
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden">
      ...
      <div className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} 
        ...
        <div className="p-6 border-t border-slate-800">
          <button
            ...
            className="w-full py-3 text-red-400 hover:bg-red-500/10 rounded-3xl transition-colors"
          >
            Logout
          </button>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="h-16 bg-slate-900 border-b border-slate-800 px-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button 
              ...
              className="flex items-center gap-2 bg-violet-600 hover:bg-violet-700 px-5 py-2.5 rounded-2xl text-sm font-medium transition active:scale-95"
            >
              <FaDownload size={18} /> 
        ...
        <div className="flex-1 p-4 overflow-auto">
          {loading && (
            <div className="text-center py-20 text-slate-400 text-lg">Loading trainees...</div>
          )}

          {error && (
            <div className="bg-red-900/30 border border-red-500 text-red-400 p-8 rounded-3xl text-center text-lg">
              {error}
            </div>
          )}

          {!loading && !error && (
            <div className="space-y-4 lg:hidden"> {/* Mobile Cards - Optimized for iPhone */}
                  ...
                  >
                    <div className="flex justify-between items-start mb-6">
                      <div>
                        ...
                        <p className="text-violet-400 font-mono text-xl mt-2 tracking-widest">
                      ...
                      <button
                        ...
                        className="bg-violet-600 hover:bg-violet-700 active:bg-violet-800 w-16 h-16 flex items-center justify-center rounded-3xl transition-all active:scale-95 shadow-lg"
                      >
                        <FaDownload size={28} />
                      </button>
                    </div>

                    <div className="space-y-2 text-sm text-slate-400 border-t border-slate-700 pt-5">
                    ...
                    <button
                      ...
                      className="mt-6 w-full py-3.5 flex items-center justify-center gap-2 text-red-400 border border-red-500/40 hover:border-red-500 rounded-2xl transition-colors font-medium"
                    >
          ...
          {/* Desktop Table - You can keep your original table here if needed */}
          <div className="hidden lg:block">
            {/* Paste your original table if you still want desktop view */}
          </div>
        </div>
      </div>
      ...
      {showDownloadError && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
          <div className="bg-slate-800 p-8 rounded-3xl w-full max-w-sm text-center border border-red-500">
            <h2 className="text-2xl font-semibold text-red-400 mb-4">Download Failed</h2>
            <p className="text-slate-300 mb-8">{downloadErrorMsg}</p>
            <button 
              onClick={() => setShowDownloadError(false)}
              className="w-full py-4 bg-red-600 hover:bg-red-500 rounded-2xl text-lg font-medium"
            >
              Close
            </button>
          </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
          <div className="bg-slate-800 p-8 rounded-3xl w-full max-w-sm">
            ...
            <p className="text-slate-300 mb-8">This action cannot be undone.</p>
            <div className="flex gap-4">
              <button 
                onClick={() => setShowDeleteModal(false)}
                className="flex-1 py-4 bg-slate-700 hover:bg-slate-600 rounded-2xl text-lg"
              >
                Cancel
              </button>
              <button 
                ...
                className="flex-1 py-4 bg-red-600 hover:bg-red-500 rounded-2xl text-lg"
              >
                Yes, Delete
              </button>
            </div>
